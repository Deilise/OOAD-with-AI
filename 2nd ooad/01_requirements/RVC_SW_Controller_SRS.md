...
This SRS delineates **what** the Roomba RVC **Software Controller** shall do to implement **automatic cleaning**: session control, surface cleaning behavior, use of direct obstacle information (**front** and **left**), inferred **right-side** obstacle status obtained by a right-side probe maneuver using available sensing, navigation and escape maneuvers (including right-side checking before **prefer-right** avoidance and, when surrounded, **reverse until a lateral opening** — **left** or **right** — then lateral escape), and dust-driven cleaning power.  
...
- **Logical** inputs (front and left obstacle state, dust signal), **derived** right-side obstacle status from a high-level right-side probe, and **logical** outputs (high-level motion and cleaning commands). Detailed motor control, register-level drivers, exact rotation angles, and electrical interfaces are **out of scope** for this SRS (see §2.4).
...
| **Obstacle (sector)** | Indication that **front**, **left**, or **right** is blocked or too close per product thresholds. **Front** and **left** are directly sensed in this revision; **right** is inferred by a right-side probe because no dedicated right-side obstacle sensor is available. |
| **Obstacle state (per sector)** | Fused obstacle/clearance for a sector. Front and left may be raw or fused direct inputs; right is the result of the right-side probe. Delivery of direct sensor samples (poll, interrupt, message, etc.) is **not** prescribed by this SRS. |
| **Right-side probe** | A high-level maneuver used to infer right-side obstacle status without a dedicated right-side sensor. The Software Controller SHALL rotate or otherwise re-orient the robot so an available sensor (for example, the front obstacle sensor) can evaluate the right side, then use that result for right-turn and surrounded decisions. Exact motor control, rotation angle, timing, and sensor hardware details are **TBD** / out of scope. |
...
| R-2 | `RVC_SW_Controller_UseCases.md` (same workspace) | Use-case companion; align revision with SRS **v0.6.0** |
...
- **Higher-level inputs:** session commands (start/stop automatic cleaning), direct **front** and **left** obstacle signals, and dust signals (**implementation-defined** direct-sample delivery). The Software Controller derives right-side status by commanding a right-side probe and evaluating available sensor data.
...
3. **Forward obstacle avoidance** — suspend cleaning; if right status is needed, perform a right-side probe using available sensing; **prefer-right** lateral turn when the probed right side is viable, otherwise turn left if viable; resume forward and cleaning.  
4. **Surrounded escape** — no forward while front+left+right are blocked, where right is determined by right-side probe; **reverse until a lateral opening** (**left** direct clearance and/or **right** probed clearance); then execute a lateral turn and resume forward/cleaning. The surrounded escape SHALL use a backward movement segment when required and SHALL NOT be replaced by a 180-degree turn followed by forward travel. **Forward-only** clearance while **both** sides remain closed **does not** end the surrounded-escape reverse **segment**; **maximum backup** + **fallback** if no lateral opening (**TBD**).  
...
7. **Consistent multi-sector decisions** — fusion/snapshot rules for combined front/left direct state and right-side probe results.
...
- **No prescribed direct sensor acquisition:** front/left obstacle signaling MAY use any combination of polling, interrupts, or messages, provided §3.2 and §3.3 are satisfied.
...
- **A-1:** Three obstacle sectors (**front**, **left**, **right**) remain meaningful for behavior, but only **front** and **left** are directly sensed in this revision. The **right** sector is evaluated through a right-side probe using available sensing; exact zones and probe pose/angle are **TBD**.  
- **A-2:** A **dust** or debris-load signal exists and can be thresholded; definition **TBD**.  
- **A-3:** A lower layer delivers direct front/left obstacle samples with bounded latency once hardware is fixed; the platform also supports high-level re-orientation commands needed for the right-side probe. Numeric targets are **TBD** (see §3.3).  
...
| SI-1 | Direct front/left obstacle and dust inputs SHALL be obtainable from a **sensor interface layer** (name/API **TBD**); acquisition mechanism (poll vs push) is **not** constrained by this SRS. |
...
| AT-3.3 | `rightObstacleInferred` | Right sector blocked / clear as determined by the right-side probe |
| AT-3.4 | `rightProbeStatus` | Pending, valid, invalid, or stale right-side probe result — exact enum **TBD** |
| AT-3.5 | `lastUpdateTime` (per direct sector, right probe, or fused) | For staleness — structure **TBD** |

#### 3.2.3.2 Functions

| ID | Functional requirement |
|----|-------------------------|
| OBJ3-FR-1 | The Software Controller SHALL obtain **front** obstacle state with latency sufficient for safe stop/avoidance and for the right-side probe when the front sensor is used during re-orientation (numeric budget **TBD**). Acquisition mechanism is **implementation-defined**. *(legacy **FR-017**)* |
...
| OBJ3-FR-3 | For decisions needing **combined** front, left, and right state, the Software Controller SHALL apply a **consistent snapshot or fusion rule** that combines direct front/left samples with the right-side probe result (ordering, debounce, staleness, and probe validity **TBD**) so behavior is not undefined. *(legacy **FR-019**)* |
...
| OBJ3-FR-5 | When direct right-side obstacle data is unavailable, the Software Controller SHALL determine right-side blockage by commanding a high-level right-side probe maneuver and evaluating the right side with available sensing before making decisions that require right-side knowledge. The probe SHALL restore or account for the original travel heading before the final avoidance/escape decision is executed; exact motion-control implementation is **TBD**. |

#### 3.2.3.3 Messages

| Direction | Message | Description |
|-----------|---------|---------------|
| Received | `ObstacleStateChanged` | Direct front/left push notification, periodic read completion, **or** probe-pose observation such as `probePose=right` using available sensing — **implementation-defined** |
...
| ID | Attribute | Description |
|----|-----------|-------------|
| AT-4.1 | `motionState` | e.g., `ForwardCruise`, `Avoiding`, `RightSideProbing`, `Reversing`, `Turning` — **TBD** |
...
| Sent | `MotionCommand` | Forward / reverse / turn / right-side probe re-orientation |
...
| DC-2 | Direct front/left obstacle acquisition (interrupt vs poll vs other) is **not** a design constraint imposed by this SRS. Right-side probe implementation details remain out of scope except for the required behavior in §3.2. |
...
| MAINT-1 | Maintainability | Obstacle thresholds, debounce windows, right-side probe parameters, max backup, dust boost duration, and latency budgets SHALL be **configurable** (compile-time or runtime) per **NFR-011** legacy intent. |
...
| OTH-4 | Design artifacts, code, tests, and simulator scenarios SHALL model the absence of a dedicated right-side obstacle sensor and SHALL trace right-side decisions to the right-side probe behavior introduced in this revision. |
